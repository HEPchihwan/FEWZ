...
try:
    ...
    for i in sectlist:
        ...
        if not os.path.isdir(subdirloc):
            os.mkdir(subdirloc)
            wrapperfile = open(subdirloc + '/wrapper.sh', 'w')
            wrapperfile.write('#!/bin/bash -e \n')
            wrapperfile.write('echo "TEST FIRST" \n')
            wrapperfile.write('PWD=`pwd` \n')
            wrapperfile.write('HOME=$PWD \n')
            wrapperfile.write('echo $HOME \n')
            wrapperfile.write('export SCRAM_ARCH=slc7_amd64_gcc900 \n')
            wrapperfile.write('source /cvmfs/cms.cern.ch/cmsset_default.sh \n')
            wrapperfile.write('cmsrel CMSSW_12_0_1 \n')
            wrapperfile.write('cd $PWD/CMSSW_12_0_1 \n')
            wrapperfile.write('ls -lrth \n')
            wrapperfile.write('eval `scramv1 runtime -sh` \n')
            wrapperfile.write('export LD_LIBRARY_PATH=/cvmfs/cms.cern.ch/slc7_amd64_gcc900/external/lhapdf/6.3.0/lib:${LD_LIBRARY_PATH} \n')
            wrapperfile.write('\n')
            wrapperfile.write('cd #PWD \n')
            wrapperfile.write('\n')
            wrapperfile.write('echo "TEST DIR" \n')
            wrapperfile.write('STEP_NUM=$1 \n')
            if sys.argv[1]=='z':
                wrapperfile.write('./fewzz -i input_z.txt -h histograms.txt -o txt -p . -s ${STEP_NUM} > screen.out \n')
            if sys.argv[1]=='w':
                wrapperfile.write('./fewzw -i input_w.txt -h histograms.txt -o txt -p . -s ${STEP_NUM} > screen.out \n')
            wrapperfile.write('rm -rf CMSSW_12_0_1')
            wrapperfile.close()
            os.chmod(subdirloc + '/wrapper.sh', 0755)
            if scalevars[2]:
